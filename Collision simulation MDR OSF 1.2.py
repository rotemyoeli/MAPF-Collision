...
def astar(array, start, goal, y, route, step):
    ...
    while oheap:
        ...
        fscore[neighbor] = gscore[current] + 1 + heuristic(neighbor[0], goal) + (np.random.random()/1)
        ...
        for i, j in neighbors:
            ...
            if tentative_g_score < gscore.get(neighbor[0], 0) or neighbor not in [i[1] for i in oheap]:
                ...
                fscore[neighbor] = tentative_g_score + heuristic(neighbor[0], goal) + (np.random.random()/1)
                heapq.heappush(oheap, (fscore[neighbor], neighbor))
